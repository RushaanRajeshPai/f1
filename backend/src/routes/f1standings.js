...
router.get('/standings', async (req, res) => {
  try {
    // Get the year from query parameters, default to 2024
    ...
    const dataPath = path.join(__dirname, '..', 'f1_standings_2020_2024.json');
    ...
    try {
      ...
      // If file exists and is less than 60 minutes old, use cached data
      if (fileAge < 60) {
        const allData = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
        // Extract only the requested year data
        const yearData = allData[requestedYear] || { drivers: [], teams: [] };
